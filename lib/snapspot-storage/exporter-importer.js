...
export class StorageExporterImporter {
  ...
  static async exportData (map, allMarkers, allPhotos, imageProcessor, options = {}, mapStorage = null) {
    ...
    if (includeMetadata && mapStorage) {
      ...
      const [mapValues, markerValuesArrays, photoValuesArrays] = await Promise.all([
        mapStorage.getMetadataValuesForEntity('map', map.id),
        Promise.all(markersToExport.map(marker =>
          mapStorage.getMetadataValuesForEntity('marker', marker.id)
        )),
        includePhotos
          ? Promise.all(photosToExport.map(photo =>
            mapStorage.getMetadataValuesForEntity('photo', photo.id)
          ))
          : Promise.resolve([])
      ])
      const markerValues = markerValuesArrays.flat()
      const photoValues = photoValuesArrays.flat()
    ...
    // Fetch marker type definitions if mapStorage available
    let markerTypeDefinitions = []

    if (mapStorage) {
      console.log('StorageExporterImporter: Fetching marker type definitions...')
      try {
        markerTypeDefinitions = await mapStorage.getAllMarkerTypeDefinitions()
        console.log(`StorageExporterImporter: Found ${markerTypeDefinitions.length} marker type definitions.`)
      } catch (error) {
        console.warn('StorageExporterImporter: Could not fetch marker type definitions:', error)
      }
    }

    // Handle split by date or single file export
    if (splitByDate && datesToExport && datesToExport.length > 1) {
      await this._exportSplitByDate(exportMap, mapImageBlob, processedMarkers, processedPhotos, datesToExport, metadataDefinitions, metadataValues, markerTypeDefinitions)
    } else {
   ...
   * @private
   */
  static async _exportSplitByDate (baseMap, mapImageBlob, allProcessedMarkers, allProcessedPhotos, datesToExport, metadataDefinitions = [], metadataValues = [], markerTypeDefinitions = []) {
  ...
  static async _processImportedDataForNewMap (importObject, ImageProcessorClass) {
    ...
    // Debug: log markerTypeId distribution in processed markers
    const markersWithType = importedMarkers.filter(m => m.markerTypeId)
    console.log('StorageExporterImporter: New map data processed.')
    console.log(`StorageExporterImporter: ${markersWithType.length}/${importedMarkers.length} markers have markerTypeId`)
    if (markersWithType.length > 0) {
      const sampleMarker = markersWithType[0]
      console.log(`StorageExporterImporter: Sample marker markerTypeId="${sampleMarker.markerTypeId}" direction=${sampleMarker.direction}`)
    }
    const mtDefs = importObject.markerTypeDefinitions || []
    console.log(`StorageExporterImporter: ${mtDefs.length} marker type definitions to import`)
    if (mtDefs.length > 0) {
      console.log('StorageExporterImporter: Type definition IDs:', mtDefs.map(d => `${d.name}(${d.id})`).join(', '))
    }
    return {
      map: importedMap,
  ...
  static async mergeData (existingMapId, importedObject, ImageProcessorClass, mapStorage, options = {}) {
    ...
    // Fetch existing marker type definitions
    const existingMarkerTypeDefs = await mapStorage.getAllMarkerTypeDefinitions()

    console.log(`StorageExporterImporter: Existing map has ${existingMarkers.length} markers, ${existingPhotos.length} photos, ${existingMetadataDefinitions.length} metadata definitions, ${existingMetadataValues.length} metadata values, ${existingMarkerTypeDefs.length} marker type definitions`)
    console.log(`StorageExporterImporter: Import has ${importedObject.markers.length} markers, ${importedObject.photos.length} photos, ${importedObject.metadataDefinitions?.length || 0} metadata definitions, ${importedObject.metadataValues?.length || 0} metadata values, ${importedObject.markerTypeDefinitions?.length || 0} marker type definitions`)
    ...
    const targetExport = {
      ...
      markerTypeDefinitions: existingMarkerTypeDefs
    }
    ...
    const sourceExport = {
      ...
      markerTypeDefinitions: importedObject.markerTypeDefinitions || []
    }
    ...
    const mergedExport = mergeExports(targetExport, sourceExport, {
      ...
      markerTypeConflictStrategy: 'skip', // Skip duplicate marker types, keep existing
    ...
    // Debug: log markerTypeId presence before saving
    const newMarkersWithType = newMarkers.filter(m => m.markerTypeId)
    console.log(`StorageExporterImporter: ${newMarkersWithType.length}/${newMarkers.length} new markers have markerTypeId`)
    if (newMarkersWithType.length > 0) {
      console.log('StorageExporterImporter: New marker type IDs:', newMarkersWithType.map(m => `${m.markerTypeId}(${m.direction ?? 'no-dir'})`).join(', '))
    }
    ...
    for (const definition of newDefinitions) {
      ...
      await mapStorage.addMetadataDefinition(definition)
    }

    for (const value of newValues) {
      await mapStorage.addMetadataValue(value)
    }

    // Save new marker type definitions
    const existingMarkerTypeIds = new Set(existingMarkerTypeDefs.map(d => d.id))
    const newMarkerTypeDefs = (mergedExport.markerTypeDefinitions || []).filter(d => !existingMarkerTypeIds.has(d.id))

    for (const typeDef of newMarkerTypeDefs) {
      // Check for duplicates by name for global definitions
      const existingByName = existingMarkerTypeDefs.find(
        d => d.name.toLowerCase() === typeDef.name.toLowerCase()
      )
      if (existingByName) {
        console.log(`StorageExporterImporter: Skipping duplicate marker type definition: ${typeDef.name}`)
        continue
      }
      try {
        await mapStorage.addMarkerTypeDefinition(typeDef)
        console.log(`StorageExporterImporter: Added marker type definition: ${typeDef.name}`)
      } catch (error) {
        console.warn(`StorageExporterImporter: Failed to add marker type definition "${typeDef.name}":`, error)
      }
    }

    console.log(
      `StorageExporterImporter: Merge complete using ${duplicateStrategy} strategy. ` +
      `Added ${newMarkers.length} markers, ${newPhotos.length} photos, ${newDefinitions.length} metadata definitions, ${newValues.length} metadata values, ${newMarkerTypeDefs.length} marker type definitions. ` +
    ...
    return {
      ...
      markerTypeDefinitions: mergedExport.markerTypeDefinitions,
      stats: {
        ...
        newMarkerTypeDefinitions: newMarkerTypeDefs.length
      }
    }
  }
   ...
   * @private
   */
  static async _saveImportedData (app, map, markers, photos, metadataDefinitions = [], metadataValues = [], markerTypeDefinitions = []) {
    ...
    for (const definition of metadataDefinitions) {
      ...
      if (!existing) {
        await app.storage.addMetadataDefinition(definition)
        if (definition.scope === 'global' && globalDefinitionNames) {
          globalDefinitionNames.add(definition.name.toLowerCase())
        }
      } else {
    ...
    // Save marker type definitions (check for conflicts by name to avoid duplicates)
    await this._saveMarkerTypeDefinitions(app, markerTypeDefinitions)
  }
   ...
   * @private
   */
  static async _deleteMapAndImportNew (app, existingMapId, newMapData, newMarkersData, newPhotosData, metadataDefinitions = [], metadataValues = [], markerTypeDefinitions = []) {
  ...
  /**
   * Helper to save marker type definitions, checking for duplicates by name.
   * Built-in definitions are always skipped.
   * @private
   */
  static async _saveMarkerTypeDefinitions (app, markerTypeDefinitions) {
    if (!markerTypeDefinitions || markerTypeDefinitions.length === 0) {
      console.log('StorageExporterImporter: No marker type definitions to save (empty or null)')
      return
    }

    console.log(`StorageExporterImporter: Saving ${markerTypeDefinitions.length} marker type definitions...`)

    // Get existing definitions for duplicate checking
    const existingDefs = await app.storage.getAllMarkerTypeDefinitions()
    console.log(`StorageExporterImporter: ${existingDefs.length} existing marker type definitions in DB`)
    ...
    for (const typeDef of markerTypeDefinitions) {
      // Skip built-in definitions (they exist in every DB)
      if (typeDef.isBuiltIn) {
        console.log(`StorageExporterImporter: Skipping built-in marker type definition: "${typeDef.name}" (${typeDef.id})`)
        skippedBuiltIn++
        continue
      }

      // Check for duplicate by name
      const existingMatch = existingByName.get(typeDef.name.toLowerCase())
      if (existingMatch) {
        console.log(`StorageExporterImporter: Skipping duplicate marker type definition "${typeDef.name}" (by name match with existing "${existingMatch.name}" id=${existingMatch.id})`)
        skippedByName++
        continue
      }

      // Check by ID
      const existingById = await app.storage.getMarkerTypeDefinition(typeDef.id)
      if (existingById) {
        console.log(`StorageExporterImporter: Skipping duplicate marker type definition "${typeDef.name}" (by ID: ${typeDef.id})`)
        skippedById++
        continue
      }
      ...
      try {
        ...
        savedCount++
        console.log(`StorageExporterImporter: Added marker type definition: "${typeDef.name}" id=${typeDef.id} shape=${typeDef.shape} color=${typeDef.color}`)
      } catch (error) {
        console.warn(`StorageExporterImporter: Failed to add marker type definition "${typeDef.name}":`, error.message || error)
        failedCount++
      }
    }

    console.log(`StorageExporterImporter: Marker type definitions saved: ${savedCount} | skipped (built-in: ${skippedBuiltIn}, by-name: ${skippedByName}, by-id: ${skippedById}) | failed: ${failedCount}`)
  }
          ...
          (async () => {
            try {
              ...
              if (importResult.importType === 'decision_required') {
                ...
                if (userChoice) {
                  ...
                  if (action === 'merge') {
                    ...
                    await this._deleteMapAndImportNew(
                      ...
                      processedNewData.markerTypeDefinitions || []
                    )
                ...
                const { map, markers, photos, metadataDefinitions = [], metadataValues = [], markerTypeDefinitions = [] } = importResult
                await this._saveImportedData(app, map, markers, photos, metadataDefinitions, metadataValues, markerTypeDefinitions)
                await app.loadMaps()
