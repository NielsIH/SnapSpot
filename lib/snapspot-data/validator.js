...
function validateMarkerTypeDefinition (definition, index) {
  ...
  // Validate showNumber (optional field)
  if ('showNumber' in definition && typeof definition.showNumber !== 'boolean') {
    errors.push(`markerTypeDefinitions[${index}].showNumber must be a boolean`)
  }
...
export function validateExportFile (exportData) {
  ...
  // Validate type field (accept all known formats)
  const validTypes = ['snapspot-export', 'SnapSpotDataExport', 'snapspot-metadata-definitions', 'snapspot-marker-type-definitions']
...
export function validateExportFileDetailed (exportData) {
  ...
  if (result.isValid) {
    if (exportData.markers && exportData.markers.length === 0) {
...
/**
 * Validate a marker type definitions-only export (snapspot-marker-type-definitions).
 *
 * Used for the separate "Export Types" / "Import Types" workflow from Settings.
 *
 * @param {Object} exportData - Parsed export data
 * @returns {{isValid: boolean, errors: string[]}} Validation result
 *
 * @example
 * const result = validateMarkerTypeDefinitionsExport(parsedJSON)
 * if (!result.isValid) {
 *   console.error('Validation errors:', result.errors)
 * }
 */
export function validateMarkerTypeDefinitionsExport (exportData) {
  const errors = []

  // Validate root structure (no map/markers required for definitions-only export)
  ...
  if (!exportData.version) {
    errors.push('export missing required field: version')
  } else if (!isSupportedVersion(exportData.version)) {
    errors.push(
      `Unsupported export version: ${exportData.version}. ` +
      `Supported versions: ${SUPPORTED_VERSIONS.join(', ')}`
    )
  ...
  if (!exportData.type) {
    errors.push('export missing required field: type')
  } else if (exportData.type !== 'snapspot-marker-type-definitions') {
    errors.push(`Invalid export type: ${exportData.type}. Expected 'snapspot-marker-type-definitions'`)
  }

  if (!exportData.sourceApp) {
    errors.push('export missing required field: sourceApp')
  }

  if (!exportData.timestamp) {
    errors.push('export missing required field: timestamp')
  }

  // Validate definitions array
  if (!exportData.definitions) {
    errors.push('export missing required field: definitions')
  } else if (!Array.isArray(exportData.definitions)) {
    errors.push('definitions must be an array')
  } else if (exportData.definitions.length === 0) {
    errors.push('definitions must contain at least one definition')
  } else {
    for (let i = 0; i < exportData.definitions.length; i++) {
      const defErrors = validateMarkerTypeDefinition(exportData.definitions[i], i)
      errors.push(...defErrors)
    }
  }
