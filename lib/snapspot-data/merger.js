...
export function mergeExports (targetExport, sourceExport, options = {}) {
  ...
  const getOrCreateMergedLineGroupId = (sourceLineGroupId) => {
    const sourceGroup = sourceLineGroupId || 'ungrouped-line-markers'
  ...
  for (const sourceMarker of sourceExport.markers) {
    if (isLineMarker(sourceMarker)) {
      console.log(`Merger: Preserving line marker from group "${sourceMarker.lineGroupId || 'unknown'}" as a new marker`)

      const newMarkerId = idGenerator('marker')
      markerIdMap.set(sourceMarker.id, newMarkerId)

      const mergedLineGroupId = getOrCreateMergedLineGroupId(sourceMarker.lineGroupId)
