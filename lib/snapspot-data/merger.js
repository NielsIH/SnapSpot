...
const DEFAULT_MARKER_TYPE = 'point'

/**
 * Determine if two markers are compatible for merging.
 *
 * Markers without an explicit `type` are treated as having type "point"
 * to remain compatible with older exports that did not store marker types.
 *
 * @param {Object} existingMarker
 * @param {Object} candidateMarker
 * @returns {boolean} True if markers are considered merge-compatible.
 */
function areMarkersMergeCompatible (existingMarker, candidateMarker) {
  const existingType = existingMarker?.type || DEFAULT_MARKER_TYPE
  const candidateType = candidateMarker?.type || DEFAULT_MARKER_TYPE
  return existingType === candidateType
}

function getLineGroupKey (marker) {
  if (marker?.lineGroupId) return marker.lineGroupId
  return `__single_line_marker__${marker?.id || ''}`
}

function groupLineMarkersByGroupId (markers) {
  const groups = new Map()

  for (const marker of markers || []) {
    if (marker?.type !== 'line') continue

    const groupKey = getLineGroupKey(marker)
    if (!groups.has(groupKey)) {
      groups.set(groupKey, [])
    }
    groups.get(groupKey).push(marker)
  }

  return groups
}

function coordinatesMatchWithinTolerance (firstMarker, secondMarker, tolerance = 0) {
  if (!firstMarker || !secondMarker) return false

  const dx = Math.abs(firstMarker.x - secondMarker.x)
  const dy = Math.abs(firstMarker.y - secondMarker.y)
  return dx <= tolerance && dy <= tolerance
}

function mapLineMarkersToCandidateGroup (sourceGroupMarkers, candidateGroupMarkers, tolerance = 0) {
  if (!Array.isArray(sourceGroupMarkers) || !Array.isArray(candidateGroupMarkers)) {
    return null
  }

  if (sourceGroupMarkers.length !== candidateGroupMarkers.length) {
    return null
  }
  ...
  for (const sourceMarker of sourceGroupMarkers) {
    let matchedIndex = -1

    for (let index = 0; index < candidateGroupMarkers.length; index++) {
      if (usedCandidateIndexes.has(index)) continue

      const candidateMarker = candidateGroupMarkers[index]
      if (coordinatesMatchWithinTolerance(sourceMarker, candidateMarker, tolerance)) {
        matchedIndex = index
        break
      }
    }
    ...
    mapping.push({
      sourceMarker,
      candidateMarker: candidateGroupMarkers[matchedIndex]
    })
  }
...
function findMatchingLineGroupWithMapping (existingLineGroups, sourceGroupMarkers, tolerance = 0) {
  for (const [, candidateGroupMarkers] of existingLineGroups) {
    const mapping = mapLineMarkersToCandidateGroup(
      sourceGroupMarkers,
      candidateGroupMarkers,
      tolerance
    )
...
export function findDuplicateMarker (existingMarkers, candidateMarker, tolerance = 0) {
  ...
  return existingMarkers.find(existing => {
    if (!areMarkersMergeCompatible(existing, candidateMarker)) {
      return false
    }
...
/**
 * Find duplicate marker type definition by ID or name+scope
 *
 * Searches for an existing marker type definition that matches by ID first,
 * then falls back to matching by name+scope if no ID match is found.
 *
 * @param {Array<Object>} existingDefinitions - Array of existing marker type definitions
 * @param {Object} candidateDefinition - Definition to search for
 * @returns {Object|null} Matching definition or null if not found
 *
 * @example
 * const match = findDuplicateMarkerTypeDefinition(existingDefs, newDef)
 */
export function findDuplicateMarkerTypeDefinition (existingDefinitions, candidateDefinition) {
  if (!Array.isArray(existingDefinitions) || !candidateDefinition) {
    return null
  }
  ...
  // Fall back to matching by name + scope (case-insensitive)
  const candidateName = (candidateDefinition.name || '').trim().toLowerCase()
  if (!candidateName) {
    return null
  }

  const candidateScope = candidateDefinition.scope || 'global'

  return existingDefinitions.find(
    def =>
      (def.name || '').trim().toLowerCase() === candidateName &&
      (def.scope || 'global') === candidateScope
  )
}

/**
 * Merge marker type definitions from two exports
 *
 * Combines target and source marker type definitions, handling duplicates
 * by matching on name+scope. Built-in definitions are never overwritten.
 *
 * @param {Array<Object>} targetDefinitions - Existing definitions
 * @param {Array<Object>} sourceDefinitions - Definitions to merge in
 * @param {Object} options - Merge options
 ...
 *
 * @example
 * const result = mergeMarkerTypeDefinitions(existingDefs, newDefs, {
 *   conflictStrategy: 'skip',
 *   idGenerator: generateId
 * })
 */
export function mergeMarkerTypeDefinitions (targetDefinitions, sourceDefinitions, options = {}) {
  const {
    conflictStrategy = 'skip',
    idGenerator = generateId
  } = options
  ...
  for (const sourceDef of sourceDefinitions) {
    // Always skip built-in definitions (they exist in every DB)
    if (sourceDef.isBuiltIn) {
      console.log(`Merger: Skipping built-in marker type definition "${sourceDef.name}"`)
      // Map source ID to itself (should match existing built-in)
      definitionIdMap.set(sourceDef.id, sourceDef.id)
      continue
    }

    const matchingDef = findDuplicateMarkerTypeDefinition(mergedDefinitions, sourceDef)

    if (matchingDef) {
      // Duplicate found - apply conflict strategy
      if (conflictStrategy === 'skip') {
        console.log(`Merger: Skipping duplicate marker type definition "${sourceDef.name}" (ID: ${sourceDef.id})`)
        // Map source ID to existing ID
        definitionIdMap.set(sourceDef.id, matchingDef.id)
      } else if (conflictStrategy === 'overwrite') {
        // Never overwrite built-in definitions
        if (matchingDef.isBuiltIn) {
          console.log(`Merger: Cannot overwrite built-in marker type definition "${sourceDef.name}"`)
          definitionIdMap.set(sourceDef.id, matchingDef.id)
        } else {
          console.log(`Merger: Overwriting marker type definition "${sourceDef.name}" (ID: ${sourceDef.id})`)
          const index = mergedDefinitions.indexOf(matchingDef)
          mergedDefinitions[index] = {
            ...sourceDef,
            lastModified: new Date().toISOString()
        ...
        console.log(`Merger: Keeping both versions of marker type definition "${sourceDef.name}"`)
        const newId = idGenerator('markerTypeDefinition')
        const newDef = {
          ...sourceDef,
          id: newId,
          name: `${sourceDef.name} (imported)`,
      ...
      // New definition - add it
      console.log(`Merger: Adding new marker type definition "${sourceDef.name}" (ID: ${sourceDef.id})`)
      const newDef = {
        ...sourceDef,
        lastModified: new Date().toISOString()
      }
 ...
 * @param {boolean} options.mergeMarkerTypes - Whether to merge marker type definitions (default: true)
 * @param {string} options.markerTypeConflictStrategy - 'skip', 'overwrite', or 'keep-both' (default: 'skip')
...
export function mergeExports (targetExport, sourceExport, options = {}) {
  const {
    ...
    mergeMarkerTypes = true,
    markerTypeConflictStrategy = 'skip',
    idGenerator = generateId
  } = options
  ...
  const mergedExport = {
    ...
    markerTypeDefinitions: targetExport.markerTypeDefinitions ? [...targetExport.markerTypeDefinitions] : [],
    metadata: {
      ...
      mergedFrom: [
        ...(targetExport.metadata?.mergedFrom || []),
        {
          ...
          markerTypeDefinitionCount: sourceExport.markerTypeDefinitions?.length || 0
        }
      ]
    }
  }
  ...
  const lineGroupIdMap = new Map()
  const sourceLineGroups = groupLineMarkersByGroupId(sourceExport.markers)
  const processedLineGroupKeys = new Set()

  // Merge marker type definitions first so we have ID mapping for marker processing
  let markerTypeIdMap = new Map()
  if (mergeMarkerTypes && sourceExport.markerTypeDefinitions && sourceExport.markerTypeDefinitions.length > 0) {
    console.log('Merger: Merging marker type definitions...')

    const markerTypeMergeResult = mergeMarkerTypeDefinitions(
      mergedExport.markerTypeDefinitions || [],
      sourceExport.markerTypeDefinitions,
      {
        conflictStrategy: markerTypeConflictStrategy,
        idGenerator
      }
    )

    mergedExport.markerTypeDefinitions = markerTypeMergeResult.definitions
    markerTypeIdMap = markerTypeMergeResult.idMap
    console.log(`Merger: Marker type definitions merged. ID map has ${markerTypeIdMap.size} entries.`)
  }

  const isLineMarker = (marker) => marker?.type === 'line'

  const getOrCreateMergedLineGroupId = (sourceLineGroupKey) => {
    const sourceGroup = sourceLineGroupKey || 'ungrouped-line-markers'

    if (lineGroupIdMap.has(sourceGroup)) {
      return lineGroupIdMap.get(sourceGroup)
    }

    const mergedGroupId = idGenerator('line-group')
    lineGroupIdMap.set(sourceGroup, mergedGroupId)
  ...
  for (const sourceMarker of sourceExport.markers) {
    if (isLineMarker(sourceMarker)) {
      const sourceLineGroupKey = getLineGroupKey(sourceMarker)
      if (processedLineGroupKeys.has(sourceLineGroupKey)) {
        continue
      }
      processedLineGroupKeys.add(sourceLineGroupKey)

      const sourceGroupMarkers = sourceLineGroups.get(sourceLineGroupKey) || [sourceMarker]
      const existingLineGroups = groupLineMarkersByGroupId([
        ...mergedExport.markers,
        ...newMarkersAdded
      ])

      const matchingLineGroup = findMatchingLineGroupWithMapping(
        existingLineGroups,
        sourceGroupMarkers,
        coordinateTolerance
      )
      ...
      if (matchingLineGroup) {
        console.log(`Merger: Skipping duplicate line group "${sourceMarker.lineGroupId || sourceLineGroupKey}"`)

        for (const pair of matchingLineGroup.mapping) {
          markerIdMap.set(pair.sourceMarker.id, pair.candidateMarker.id)
        }

        continue
      }

      console.log(`Merger: Adding new line group "${sourceMarker.lineGroupId || sourceLineGroupKey}"`)
      const mergedLineGroupId = getOrCreateMergedLineGroupId(sourceLineGroupKey)

      for (const sourceLineMarker of sourceGroupMarkers) {
        const newMarkerId = idGenerator('marker')
        markerIdMap.set(sourceLineMarker.id, newMarkerId)

        const sourcePhotosForMarker = sourceExport.photos.filter(
          p => p.markerId === sourceLineMarker.id
        )
        ...
        const newLineMarker = {
          ...sourceLineMarker,
          id: newMarkerId,
          lineGroupId: mergedLineGroupId,
          photoIds: newPhotoIdsForMarker,
          createdDate: preserveTimestamps
            ? sourceLineMarker.createdDate
            : new Date().toISOString(),
          lastModified: new Date().toISOString()
        }

        // Remap markerTypeId if present
        if (newLineMarker.markerTypeId && markerTypeIdMap.has(newLineMarker.markerTypeId)) {
          newLineMarker.markerTypeId = markerTypeIdMap.get(newLineMarker.markerTypeId)
        }

        newMarkersAdded.push(newLineMarker)
      }
    ...
    if (matchingMarker) {
      ...
      // Remap markerTypeId if present
      if (newMarker.markerTypeId && markerTypeIdMap.has(newMarker.markerTypeId)) {
        newMarker.markerTypeId = markerTypeIdMap.get(newMarker.markerTypeId)
      }
  ...
  if (mergeMarkerTypes) {
    console.log(`Merger: Marker type definitions count: ${mergedExport.markerTypeDefinitions?.length || 0}`)
  }
...
export function getMergeStatistics (targetExport, sourceExport, options = {}) {
  ...
  let newPhotos = 0
  const sourceLineGroups = groupLineMarkersByGroupId(sourceExport.markers)
  const targetLineGroups = groupLineMarkersByGroupId(targetExport.markers)
  const processedLineGroupKeys = new Set()

  for (const sourceMarker of sourceExport.markers) {
    if (sourceMarker?.type === 'line') {
      const sourceLineGroupKey = getLineGroupKey(sourceMarker)
      if (processedLineGroupKeys.has(sourceLineGroupKey)) {
        continue
      }
      ...
      if (matchingLineGroup) {
        duplicateMarkers += sourceGroupMarkers.length
      } else {
        newMarkers += sourceGroupMarkers.length
      }

      for (const sourceGroupMarker of sourceGroupMarkers) {
        const sourcePhotosForMarker = sourceExport.photos.filter(
          p => p.markerId === sourceGroupMarker.id
        )

        if (matchingLineGroup) {
          const mappedPair = matchingLineGroup.mapping.find(pair => pair.sourceMarker.id === sourceGroupMarker.id)
          const matchedMarkerId = mappedPair?.candidateMarker?.id
