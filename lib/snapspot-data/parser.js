...
export async function parseExport (jsonString) {
  ...
  // Extract marker type definitions (v1.3+)
  const markerTypeDefinitions = exportData.markerTypeDefinitions || []

  // Validate marker references to marker type definitions
  if (markerTypeDefinitions.length > 0) {
    const knownTypeIds = new Set(markerTypeDefinitions.map(def => def.id))
    for (let i = 0; i < exportData.markers.length; i++) {
      const marker = exportData.markers[i]
      if (marker.markerTypeId && !knownTypeIds.has(marker.markerTypeId)) {
        warnings.push({
          type: 'unknown-marker-type',
          markerIndex: i,
          markerTypeId: marker.markerTypeId,
          message: `Marker at index ${i} references unknown markerTypeId: ${marker.markerTypeId}`
        })
      }
    }
  }
  ...
  return {
    ...
    // Metadata definitions array (v1.2+)
    metadataDefinitions: exportData.metadataDefinitions || [],

    // Metadata values array (v1.2+)
    metadataValues: exportData.metadataValues || [],

    // Marker type definitions array (v1.3+)
 ...
 * @returns {number} return.metadataDefinitionCount - Number of metadata definitions (v1.2+)
 * @returns {number} return.metadataValueCount - Number of metadata values (v1.2+)
 * @returns {number} return.markerTypeDefinitionCount - Number of marker type definitions (v1.3+)
...
export function parseExportMetadata (jsonString) {
  ...
  return {
    ...
    metadataDefinitionCount: exportData.metadataDefinitions?.length || 0,
    metadataValueCount: exportData.metadataValues?.length || 0,
    markerTypeDefinitionCount: exportData.markerTypeDefinitions?.length || 0,
